...
import { useFarmDataSelector } from 'src/context/farm-context'
...
export default function FarmedTokenListDialog({
  openDialog,
  handleDialogClose,
  tokenList,
}) {
  const switchPid = useFarmDataSelector((state) => state.switchPid)
        ...
        {tokenList?.slice(page * rowsPerPage, (page + 1) * rowsPerPage)?.map((token, idx) => {
          return (
            <Box
              ...
              onClick={() => {
                switchPid(idx + page * rowsPerPage)
                handleDialogClose()
              }}
            >
              <Box>
